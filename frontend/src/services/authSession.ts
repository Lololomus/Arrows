...
function getAuthExpiredMessage(): string {
...
function resolveAuthFailureMessage(error: unknown): string {
  ...
  return getAuthExpiredMessage();
}

async function performTelegramAuth(): Promise<void> {
  const initData = getTelegramInitData();
  if (!initData) {
    throw new Error(getAuthExpiredMessage());
  }
  ...
  if (!response.ok) {
    const detail = response.status === 401
      ? getAuthExpiredMessage()
      : typeof data === 'object' && data !== null && 'detail' in data
        ? String((data as { detail?: unknown }).detail || getAuthExpiredMessage())
        : getAuthExpiredMessage();
...
export async function ensureFreshSession(): Promise<void> {
  ...
  if (state.authStatus === 'expired') {
    throw new Error(state.authMessage || getAuthExpiredMessage());
  }
