"""
Broadcast: New Season announcement to all users.

Run at 17:00 MSK (14:00 UTC) on 2026-04-02:
    cd backend && python scripts/broadcast_new_season.py

Sends the new-season message to every non-banned user who has a telegram_id.
...
from app.services.bot_notifications import notify_new_season
...
async def broadcast() -> None:
    ...
    total = len(rows)
    logger.info("Broadcasting new-season notification to %d users", total)

    sent = blocked = failed = 0

    for i, (telegram_id, locale) in enumerate(rows, start=1):
        delivery = await notify_new_season(telegram_id, locale)
        if delivery == "sent":
            sent += 1
